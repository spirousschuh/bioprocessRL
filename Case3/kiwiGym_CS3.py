    ...
    def __init__(self,render_mode=None,model_parameters0=[]):
        ...
        for i in range(self.num_experiments):
            ...
            feed_profiles[i]={'time_pulse':self.pulses_time.tolist(),'Feed_pulse':feed_profile_i.tolist(),'time_sample':np.arange(self.final_time)+self.sample_offsets[i],
                   'time_sensor':np.linspace(0.04,self.final_time,1*25*round(self.final_time))}
        ...
        self.obs=np.zeros([self.control_inputs[0][0]*(4+25)])
        return
# %%    
    def reset(self, seed=None,model_parameters=[]):
        ...
        self.state=deepcopy(initial_states)
        self.feed_profiles_history=deepcopy(self.feed_profiles)
        self.obs=np.zeros([self.control_inputs[0][0]*(4+25*0+1)])
        self.terminated=False
        return 
# %%    
    def render(self):
        ...
        for i2 in range(self.control_inputs[0][0]):
            plt.plot(self.state['sample'][i2][3],'.')#self.feed_profiles[i2]['time_sensor'],
        plt.show()
        for i2 in range(self.control_inputs[0][0]):
    ...
    def perform_action(self,action_step=[]):
        ...
        if len(action_step)==0:
            ...
            for i in range(self.control_inputs[0][0]):
                ...
                feed_profiles_corrected[(t_pulse>=t_pulse[0]) & (feed_profiles_corrected<5)]=5 #After that, min 5 ul
        ...
        if self.current_time>=self.final_time:
            ...
            # #Information gain
            THsd0=self.model_parameters[0:18]*0+.1
            THsd0[13]=.5
            THsd0[14]=.5
            THsd0[15]=.5
            THsd0[16]=.5
            ...
            Si,Q,FIM,state,traceFIM,FIM_crit,eij=method_kiwiGym.calculate_FIM(np.array([0,self.final_time]),self.initial_states,self.control_inputs,self.model_parameters,self.feed_profiles_history,C2,Cov_TH0)
